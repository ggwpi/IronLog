...
function liveWorkoutFor(displayWorkout, workouts) {
  return workouts.find((item) => item.id === displayWorkout.id) || null;
}
...
function hero(workout, workouts, activeSession) {
  ...
  return `<section class="training-hero" aria-label="האימון הנבחר">
    <div class="training-hero__copy">
      ...
      <button class="training-start" type="button" data-start-workout="${templateId}">
...
function programRow(label, status, stateClass, workouts, progressText = '') {
  ...
  return `<button class="training-plan-row ${stateClass}" type="button" ${action}>
...
function planList(workouts) {
  return `<section class="training-plan" aria-label="תוכנית האימונים שלך">
    ...
    <div class="training-plan__rows">
      ...
      ${programRow('LEGS', 'רגליים חלקית', 'is-partial', workouts, 'רגליים חלקית')}
      ${programRow('ARMS', 'הבא בתור', 'is-next', workouts, 'הבא בתור')}
    </div>
  </section>`;
}
...
// Kept for app.js compatibility. The old workouts UI does not expose the builder inline.
export function WorkoutBuilderExerciseRow() {
  return '';
}

export function WorkoutsScreen({ workouts = WORKOUTS, workoutData = {} } = {}) {
  ...
  return `<div class="workouts-concept animate-enter" dir="rtl">
    ...
    ${hero(workout, workouts, workoutData.activeSession || null)}
